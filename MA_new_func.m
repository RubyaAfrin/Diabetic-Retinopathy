function [ HM_img,HM_area ] = MA_new_func( input_img )
...
%HM1 =  img_open_2-bw1-bw2;
HM = ( MA_p-img_open_2-bw1);
MA = bwareaopen(HM,10);
HM_area_n = sum(HM(:));
%MA =   MA_p - HM -bw1  ;
%figure,imshow(HM),title('MA');
%figure,imshow(BW2),title('BW2');
%figure,imshow(HM),title('HM');
bwf= bwareaopen(bw, 200);
area1 = bwarea(bw);
area2 = bwarea(bwf);                    % Morphological Open
fprintf('New MA is : %d ', HM_area_n);
HM_area = bwarea(HM);
fprintf(' MA is : %d ', HM_area);
%disp(HM_area);
